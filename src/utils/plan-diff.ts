...
import type { PlanDiff, PlanComparison } from '../types';
...
export class PlanDiffAnalyzer {
  ...
  static comparePlans(planA: PlanFile, planB: PlanFile): PlanComparison {
    ...
    this.compareBasicProperties(planA, planB, differences);
    
    // Compare dependencies
    this.compareDependencies(planA, planB, differences);
    
    // Compare artifacts
    this.compareArtifacts(planA, planB, differences);
    
    // Compare model shape
    this.compareModelShape(planA, planB, differences);
    
    // Compare resources
    this.compareResources(planA, planB, differences);
    
    // Compare build steps
    this.compareBuildSteps(planA, planB, differences);
  ...
  private static compareBasicProperties(planA: PlanFile, planB: PlanFile, differences: PlanDiff[]): void {
    const properties = [
    ...
    for (const { field, category } of properties) {
      ...
      if (valueA !== valueB) {
        differences.push({
          ...
          oldValue: valueA,
          newValue: valueB,
          impact: this.assessImpact(field, valueA, valueB),
  ...
  private static compareDependencies(planA: PlanFile, planB: PlanFile, differences: PlanDiff[]): void {
    ...
    for (const [name, depA] of depsA) {
      ...
      if (depB && depA.version !== depB.version) {
        differences.push({
          ...
          impact: this.assessVersionChangeImpact(depA.version, depB.version),
  ...
  private static compareArtifacts(planA: PlanFile, planB: PlanFile, differences: PlanDiff[]): void {
    ...
    for (const [path, artifactA] of artifactsA) {
      ...
      if (artifactB && artifactA.estimatedSize !== artifactB.estimatedSize) {
        ...
        const percentChange = ((sizeDiff / artifactA.estimatedSize) * 100).toFixed(1);
        ...
        differences.push({
          ...
          impact: Math.abs(parseFloat(percentChange)) > 50 ? 'high' : 'medium',
          description: `Artifact '${path}' size changed by ${percentChange}% (${this.formatBytes(sizeDiff)})`
        });
      }
    }
  }

  private static compareModelShape(planA: PlanFile, planB: PlanFile, differences: PlanDiff[]): void {
    ...
    for (const prop of modelProperties) {
      ...
      if (valueA !== valueB) {
        differences.push({
          ...
          oldValue: valueA,
          newValue: valueB,
          impact: this.assessModelChangeImpact(prop, valueA, valueB),
  ...
  private static compareResources(planA: PlanFile, planB: PlanFile, differences: PlanDiff[]): void {
    ...
    for (const prop of resourceProps) {
      ...
      if (valueA !== valueB) {
        const change = typeof valueA === 'number' && typeof valueB === 'number' 
        ...
        differences.push({
          ...
          oldValue: valueA,
          newValue: valueB,
          impact: this.assessResourceChangeImpact(prop, valueA, valueB),
  ...
  private static compareBuildSteps(planA: PlanFile, planB: PlanFile, differences: PlanDiff[]): void {
  ...
  private static assessImpact(field: string, _oldValue: any, _newValue: any): 'low' | 'medium' | 'high' {
  ...
  private static assessDependencyImpact(dep: any): 'low' | 'medium' | 'high' {
    ...
    return criticalDeps.some(critical => dep.name.toLowerCase().includes(critical.toLowerCase())) ? 'high' : 'medium';
  }

  private static assessVersionChangeImpact(oldVersion: string, newVersion: string): 'low' | 'medium' | 'high' {
  ...
  private static assessModelChangeImpact(prop: string, _oldValue: any, _newValue: any): 'low' | 'medium' | 'high' {
  ...
  private static assessResourceChangeImpact(_prop: string, oldValue: any, newValue: any): 'low' | 'medium' | 'high' {
  ...
  private static generateSummary(differences: PlanDiff[]): PlanComparison['summary'] {
  ...
  static formatComparison(comparison: PlanComparison, useColors = true): string {
    ...
    lines.push(colorize(`  • Total changes: ${comparison.summary.totalChanges}`, chalk.cyan));
    lines.push(colorize(`  • High impact changes: ${comparison.summary.highImpactChanges}`, comparison.summary.highImpactChanges > 0 ? chalk.red : chalk.green));
    
    // Category breakdown
    if (Object.keys(comparison.summary.categoryCounts).length > 0) {
      lines.push('  • Changes by category:');
      for (const [category, count] of Object.entries(comparison.summary.categoryCounts)) {
    ...
    if (comparison.differences.length > 0) {
      ...
      for (const [category, diffs] of Object.entries(byCategory)) {
        lines.push(colorize(`  ${this.getCategoryIcon(category)} ${category.toUpperCase()}:`, chalk.bold));
