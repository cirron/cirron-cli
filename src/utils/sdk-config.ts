...
function coerceSnapshot(raw: unknown): string | null {
  if (typeof raw === 'string' && VALID_SNAPSHOTS.has(raw)) return raw;
  return null;
}
...
export function readHomeConfigToml(homedir: string = os.homedir()): TomlLayer {
  ...
  if (!defaultTable || typeof defaultTable !== 'object' || Array.isArray(defaultTable)) {
    return layer;
  }
  ...
  for (const [key, coerce] of Object.entries(COERCERS)) {
    if (key in table) {
      ...
      if (coerced !== null) layer.values[key] = coerced;
    }
  }
  return layer;
}
...
export function resolveSdkConfig(options?: {
  ...
  function resolveField<T>(
    ...
    if (key in toml.values) {
      return { value: toml.values[key] as T, source: 'config.toml', envVar: null };
    }
  ...
  return {
    ...
    workspaceId: resolveField<string | null>('workspace_id', DEFAULTS.workspace_id),
    ...
    flushInterval: resolveField<number>('flush_interval', DEFAULTS.flush_interval),
    spoolMaxBytes: resolveField<number>('spool_max_bytes', DEFAULTS.spool_max_bytes),
    ingestPath: resolveField<string>('ingest_path', DEFAULTS.ingest_path),
    loadWarnBytes: resolveField<number>('load_warn_bytes', DEFAULTS.load_warn_bytes),
    loadMaxBytes: resolveField<number>('load_max_bytes', DEFAULTS.load_max_bytes),
