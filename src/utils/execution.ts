    ...
    if (!inTraceback) {
      ...
      if (importMatch && importMatch[1] && importMatch[2]) {
        errors.push({
          ...
          traceback: [line],
          suggestions: generateSuggestions('import', importMatch[2], importMatch[1])
        });
      }
    }
  }
...
function generateSuggestions(errorType: ParsedError['type'], message: string, exceptionName?: string): string[] {
  ...
  if (errorType === 'import' || exceptionName?.includes('ModuleNotFound')) {
    ...
    if (lowerMessage.includes('torch')) {
      ...
      suggestions.push('For GPU support: pip install torch torchvision --index-url https://download.pytorch.org/whl/cu118');
    ...
    } else if (lowerMessage.includes('sklearn') || lowerMessage.includes('scikit')) {
    ...
    } else if (lowerMessage.includes('cv2') || lowerMessage.includes('opencv')) {
      suggestions.push('Try running: pip install opencv-python');
    } else if (lowerMessage.includes('pil') || lowerMessage.includes('pillow')) {
  ...
  if (lowerMessage.includes('cuda')) {
    if (lowerMessage.includes('out of memory')) {
      ...
      suggestions.push('Install CUDA toolkit: https://developer.nvidia.com/cuda-downloads');
  ...
  if (errorType === 'runtime' || errorType === 'exception') {
    if (lowerMessage.includes('device-side assert')) {
      ...
      suggestions.push('Add torch.cuda.synchronize() to get clearer error location');
    } else if (lowerMessage.includes('shape') || lowerMessage.includes('dimension')) {
    ...
    } else if (lowerMessage.includes('index') && lowerMessage.includes('out of range')) {
...
export function formatExecutionError(result: ExecutionResult, showDetails = false, useColors = true): string {
  const parts: string[] = [];
  ...
  if (result.parsedErrors && result.parsedErrors.length > 0) {
    ...
    result.parsedErrors.forEach((error, index) => {
      ...
      parts.push(colorize(`  ${index + 1}. ${error.type.toUpperCase()}: `, chalk.red) + 
...
function formatTracebackLine(line: string, useColors = true): string {
  ...
  const exceptionMatch = line.match(/^(\w+(?:Error|Exception|Warning)):\s*(.*)$/);
  if (exceptionMatch && exceptionMatch[1] && exceptionMatch[2]) {
...
export function formatExecutionResultAsJSON(result: ExecutionResult, includeRaw = false): string {
  const jsonResult: any = {
    ...
    errors: result.parsedErrors?.map(error => ({
      type: error.type,
...
export function logExecutionResult(result: ExecutionResult, options: { 
  jsonMode?: boolean; 
  showDetails?: boolean; 
  includeRaw?: boolean;
} = {}): void {
