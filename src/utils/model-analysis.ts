...
export class ModelAnalyzer {
      ...
      if (modelConfig) {
        ...
        if (modelConfig.outputShape) {
          analysis.outputShape =
            typeof modelConfig.outputShape === "string"
    ...
    try {
      ...
      if (await this.canRunDynamicAnalysis()) {
        ...
        analysis.warnings.push(
          "Could not perform dynamic analysis - model may have missing dependencies"
        );
      }
  ...
  private async analyzeModelCode(
    content: string,
    ...
    if (inputShape) {
      analysis.inputShape = inputShape;
    }
    if (outputShape) {
      analysis.outputShape = outputShape;
    }
  }
  ...
  private detectArchitectureFromCode(content: string): string {
    const patterns = {
      ...
      VGG: ["VGG", "vgg", "make_layers"],
      "U-Net": ["UNet", "unet", "encoder", "decoder", "upsampl", "downsampl"],
      GAN: [
        "Generator",
        "Discriminator",
        "generator",
    ...
    const bestMatch = Object.entries(scores).reduce(
      (best, [arch, score]) => (score > best[1] ? [arch, score] : best),
      ["Unknown", 0]
    );
  ...
  private estimateSklearnParameters(modelType: string): number {
    ...
    const estimates: Record<string, number> = {
      RandomForestClassifier: 50_000,
      RandomForestRegressor: 50_000,
      SVM: 10_000,
      SVC: 10_000,
      SVR: 10_000,
      LogisticRegression: 1000,
      LinearRegression: 1000,
      GradientBoostingClassifier: 30_000,
      GradientBoostingRegressor: 30_000,
    };

    return estimates[modelType] || 5000;
  }

  private estimateParametersFromCode(
    content: string,
    analysis: ModelAnalysis
  ): void {
    // Sum up parameters from all layers
    analysis.totalParameters = analysis.layers.reduce(
    ...
    if (content.includes("BatchNorm") || content.includes("batch_norm")) {
      ...
      analysis.nonTrainableParameters = Math.floor(
        analysis.totalParameters * 0.1
      );
    }
  ...
  private detectInputShape(
    ...
    if (content.includes("Linear") || content.includes("Dense")) {
...
`;
  ...
  private async analyzePyTorchDynamically(
    analysis: ModelAnalysis
  ): Promise<void> {
    const script = `
import sys
...
`;
    ...
    try {
      ...
      if (result.success && result.stdout) {
        ...
        if (!modelInfo.error) {
          analysis.totalParameters =
            modelInfo.total_parameters || analysis.totalParameters;
          analysis.trainableParameters =
            modelInfo.trainable_parameters || analysis.trainableParameters;
          analysis.nonTrainableParameters =
            modelInfo.non_trainable_parameters ||
            analysis.nonTrainableParameters;
      ...
      analysis.warnings.push(
        `Dynamic PyTorch analysis failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }

  private async analyzeTensorFlowDynamically(
    analysis: ModelAnalysis
  ): Promise<void> {
    const script = `
import sys
...
`;
    ...
    try {
      ...
      if (result.success && result.stdout) {
        ...
        if (!modelInfo.error) {
          ...
          analysis.trainableParameters =
            modelInfo.trainable_parameters || analysis.trainableParameters;
          analysis.nonTrainableParameters =
            analysis.totalParameters - analysis.trainableParameters;
          analysis.inputShape = modelInfo.input_shape || analysis.inputShape;
      ...
      analysis.warnings.push(
        `Dynamic TensorFlow analysis failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }

  private async analyzeSklearnDynamically(
    analysis: ModelAnalysis
  ): Promise<void> {
...
`;
    ...
    try {
      ...
      if (result.success && result.stdout) {
        ...
        if (!modelInfo.error) {
          ...
          analysis.totalParameters = this.estimateSklearnParameters(
      ...
      analysis.warnings.push(
        `Dynamic scikit-learn analysis failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private calculateComplexity(
    ...
    const paramThresholds = {
      low: 100_000, // < 100K parameters
      medium: 10_000_000, // < 10M parameters
      high: Number.POSITIVE_INFINITY, // >= 10M parameters
    };

    if (analysis.totalParameters < paramThresholds.low) {
      return "low";
    }
    if (analysis.totalParameters < paramThresholds.medium) {
      return "medium";
    }
    return "high";
  }
