...
export async function infoCommand(options: InfoOptions = {}): Promise<void> {
  try {
    ...
    if (!projectConfigResult) {
      logger.error(
        "Not a Cirron project. Run this command in a directory with cirron.yaml or cirron.json"
      );
      process.exit(1);
    }
    ...
    if (fs.existsSync(modelPyPath)) {
      ...
      modelAnalysis = await analyzeModelFile(
        modelContent,
        projectConfig.framework || "custom"
      );
    }
    ...
    if (options.update) {
      ...
      logger.error(
        `Unknown update type: ${options.update}. Available options: metadata`
      );
      process.exit(1);
    }
...
async function analyzeModelFile(
  ...
  for (const line of lines) {
    ...
    if (classMatch && classMatch[1]) {
      ...
      if (
        inheritance.includes("nn.Module") ||
    ...
    const architecturePatterns = [
      ...
      {
        pattern: /Transformer|nn\.Transformer|MultiheadAttention/i,
        type: "Transformer",
      },
      { pattern: /ResNet|residual|skip.*connection/i, type: "ResNet" },
      {
        pattern:
          /BatchNorm|nn\.BatchNorm|tf\.keras\.layers\.BatchNormalization/i,
        type: "BatchNorm",
      },
      {
        pattern: /Dropout|nn\.Dropout|tf\.keras\.layers\.Dropout/i,
        type: "Regularization",
      },
      { pattern: /Attention|attention|self\.attn/i, type: "Attention" },
      {
        pattern: /Embedding|nn\.Embedding|tf\.keras\.layers\.Embedding/i,
        type: "Embedding",
      },
    ];
    ...
    for (const { pattern, type } of architecturePatterns) {
      if (
        ...
        !analysis.architecturePatterns.includes(type)
      ) {
...
async function analyzeFrameworkSpecific(
  line: string,
  framework: string,
...
async function analyzeTensorFlow(
  line: string,
  analysis: ModelAnalysis
): Promise<void> {
...
async function analyzeSklearn(
  ...
  if (line.includes(".fit(")) {
    analysis.outputShapes.push(
      "Sklearn model (shape depends on training data)"
    );
  }
}
...
function extractModelInfo(
  ...
  if (modelAnalysis) {
    ...
    if (
      modelAnalysis.parameterCounts.length > 0 ||
      modelAnalysis.modelDefinitions.length > 0
    ) {
      info.params = estimateParameterCount(
        projectConfig.framework || "custom",
        modelAnalysis
      );
    }
  ...
  if (projectConfig.metadata) {
    ...
    if (!info.inputShape && projectConfig.metadata.inputShape) {
      info.inputShape =
        typeof projectConfig.metadata.inputShape === "string"
  ...
  switch (framework) {
    case "pytorch":
      if (analysis.modelDefinitions.length > 0) {
        return 1_000_000; // Rough estimate for a typical PyTorch model
      }
      break;
    case "tensorflow":
      if (analysis.modelDefinitions.length > 0) {
        return 500_000; // Rough estimate for a typical TensorFlow model
      }
      break;
    case "sklearn":
      if (analysis.modelDefinitions.length > 0) {
...
function displayModelConfigInfo(modelConfig: ModelConfig): void {
  ...
  console.log(
    `  ${chalk.yellow("Name:")} ${modelConfig.name || "Not specified"}`
  );
  console.log(
    `  ${chalk.yellow("Architecture:")} ${modelConfig.architecture || "Not specified"}`
  );
  console.log(
    `  ${chalk.yellow("Framework:")} ${modelConfig.framework || "Not specified"}`
  );

  if (modelConfig.parameters) {
    console.log(
      `  ${chalk.yellow("Total Parameters:")} ${modelConfig.parameters.total?.toLocaleString() || "Unknown"}`
    );
  }

  if (modelConfig.inputShape) {
    const inputShape =
      typeof modelConfig.inputShape === "string"
  ...
  if (modelConfig.inference?.device) {
    console.log(
      `  ${chalk.yellow("Target Device:")} ${modelConfig.inference.device}`
    );
  }

  if (modelConfig.metadata?.description) {
    console.log(
      `  ${chalk.yellow("Description:")} ${modelConfig.metadata.description}`
    );
  }
...
function displayModelInfo(projectName: string, info: ModelInfo): void {
  ...
  if (info.gpuRequired !== undefined) {
    console.log(
      `  ${chalk.yellow("GPU Required:")} ${info.gpuRequired ? "Yes" : "No"}`
    );
  }
  ...
  if (info.params !== undefined && info.params > 0) {
    ...
    console.log(
      `  ${chalk.yellow("Parameters:")} ${chalk.gray("Unable to determine (run with model loaded for accurate count)")}`
    );
  }
  ...
  if (info.trainingDataShape && info.testDataShape) {
    console.log(
      `  ${chalk.yellow("Training Data Shape:")} ${info.trainingDataShape}`
    );
  ...
  if (info.outputShape) {
    ...
    console.log(
      `  ${chalk.yellow("Input/Output Shape:")} ${chalk.gray("Not detected")}`
    );
  }
  ...
  if (info.dependencies && info.dependencies.length > 0) {
    ...
    console.log(chalk.bold("Key Dependencies"));
    console.log(
      `  ${chalk.gray("Check requirements.txt for full dependency list")}`
    );
  }
  ...
  console.log(
    chalk.gray("Tip: Run ") +
      chalk.cyan("cirron test") +
      chalk.gray(" to validate your model setup and dependencies")
  );
}
...
async function handleMetadataUpdate(
  ...
  try {
    ...
    if (changes.length === 0) {
      console.log(
        chalk.green("No metadata changes found. Project config is up to date.")
      );
      return;
    }
    ...
    if (dryRun) {
      console.log(
        chalk.blue("Dry run mode - no changes were made to the project config")
      );
    ...
    if (currentStats.mtime.getTime() !== originalModTime.getTime()) {
      logger.error(
        "Project config has been modified by another process. Please retry the update."
      );
      process.exit(1);
    }
    ...
    console.log(
      chalk.green(
        `Successfully updated ${changes.length} metadata field(s) in ${configFilename}`
      )
    );
  } catch (error) {
...
function generateUpdatedMetadata(
  ...
  if (modelAnalysis) {
    ...
    if (
      modelAnalysis.modelClassNames.length > 0 &&
      modelAnalysis.modelClassNames[0]
    ) {
...
function compareMetadata(
  ...
  if (
    JSON.stringify(oldPatterns.sort()) !== JSON.stringify(newPatterns.sort())
  ) {
    changes.push({
...
function detectMetadataMismatches(
  ...
  if (modelAnalysis.modelClassNames.length > 0) {
    ...
    if (
      detectedClassName &&
      storedClassName &&
      detectedClassName !== storedClassName
    ) {
      mismatches.push({
  ...
  if (modelAnalysis.architecturePatterns.length > 0) {
    ...
    if (JSON.stringify(detectedPatterns) !== JSON.stringify(storedPatterns)) {
      ...
      const storedArch =
        storedPatterns.length > 0 ? storedPatterns.join(" + ") : "none";
  ...
  if (modelAnalysis.inputShapes.length > 0) {
    ...
    const storedShape =
      typeof metadata.inputShape === "string" ? metadata.inputShape : undefined;

    if (
      storedShape &&
      detectedShape !== storedShape &&
      !storedShape.includes("Varies") &&
      !storedShape.includes("Not specified")
    ) {
      mismatches.push({
        field: "inputShape",
...
function displayMismatchWarnings(mismatches: MetadataMismatch[]): void {
  ...
  for (const mismatch of mismatches) {
    ...
    console.log(
      `  ${chalk.yellow("•")} ${severityColor(severityFlag)} ${mismatch.description}`
    );
  }
