...
async function configCommand(_options: HardwareOptions): Promise<void> {
  ...
  // Save configuration
  const { shouldSave, shouldApplyToProject } = await inquirer.prompt([
    {
      type: 'confirm',
      name: 'shouldSave',
    ...
    },
    {
      ...
      default: true,
      when: () => fs.existsSync('cirron.json')
    }
  ]);
...
async function interactiveCommand(options: HardwareOptions): Promise<void> {
  ...
  switch (answers.action) {
    case 'detect':
      ...
      break;
    case 'config':
      await configCommand({ ...options, config: true });
      break;
    case 'list':
