...
async function detectCommand(options: HardwareOptions): Promise<void> {
  ...
  try {
    ...
    logger.info(
      `${chalk.cyan("Architecture:")} ${hardwareConfig.architecture}`
    );
    ...
    if (hardwareConfig.specifications.cpu) {
      ...
      logger.info(
        `  Architecture: ${hardwareConfig.specifications.cpu.architecture}`
      );
    }
    ...
    if (hardwareConfig.specifications.memory) {
      ...
      logger.info(
        `  Available: ${hardwareConfig.specifications.memory.available}`
      );
    }
    ...
    if (hardwareConfig.specifications.cuda) {
      ...
      logger.info(
        `  Available: ${hardwareConfig.specifications.cuda.available ? chalk.green("Yes") : chalk.red("No")}`
      );
    ...
    logger.info(
      `  PyTorch: ${hardwareConfig.compatibility.pytorch ? chalk.green("✓") : chalk.red("✗")}`
    );
    logger.info(
      `  TensorFlow: ${hardwareConfig.compatibility.tensorflow ? chalk.green("✓") : chalk.red("✗")}`
    );
    logger.info(
      `  Scikit-learn: ${hardwareConfig.compatibility.sklearn ? chalk.green("✓") : chalk.red("✗")}`
    );

    // Requirements and warnings
    if (
      hardwareConfig.compatibility.requirements &&
      hardwareConfig.compatibility.requirements.length > 0
    ) {
      console.log();
    ...
    if (options.save) {
      ...
      logger.info(
        `  ${chalk.cyan(`cirron config hardware --configure --from ${configPath}`)}`
      );
...
async function configCommand(options: HardwareOptions): Promise<void> {
  ...
  if (options.from) {
    try {
      const loadedConfig = await HardwareDetector.loadHardwareConfig(
        options.from
      );
      if (!loadedConfig) {
  ...
  if (!options.from) {
    questions.push({
      ...
      message: isProjectDirectory
        ? "Also save hardware configuration to separate file?"
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      ...
      validate: (input: string) =>
        input.trim().length > 0 || "Architecture is required",
    },
  ]);
...
async function loadExistingConfig(): Promise<void> {
  try {
    ...
    if (!config) {
      ...
      logger.info(
        "Run " +
          chalk.cyan("cirron config hardware --configure") +
          " to create one"
      );
      return;
    }
...
function displayConfigSummary(config: HardwareConfig): void {
  ...
  if (config.specifications.cpu) {
    logger.info(
      `${chalk.cyan("CPU:")} ${config.specifications.cpu.model} (${config.specifications.cpu.cores} cores)`
    );
  }

  if (config.specifications.gpu) {
    logger.info(
      `${chalk.cyan("GPU:")} ${config.specifications.gpu.model} (${config.specifications.gpu.memory})`
    );
  }

  if (config.specifications.cuda?.available) {
    logger.info(
      `${chalk.cyan("CUDA:")} v${config.specifications.cuda.version} (${config.specifications.cuda.devices.length} devices)`
    );
  }

  logger.info(
    `${chalk.cyan("Framework Support:")} ${Object.entries(config.compatibility)
...
async function applyToProject(hardwareConfig: HardwareConfig): Promise<void> {
  try {
    ...
    const { configPath: projectConfigPath, config: projectConfig } =
      projectConfigResult;
