...
export async function configCommand(
  ...
  try {
    ...
    if (scope === "cli") {
      ...
      if (options.list) {
        cliOpts.list = options.list;
      }
      if (options.get) {
        cliOpts.get = options.get;
      }
      if (options.set) {
        cliOpts.set = options.set;
      }
      if (options.delete) {
        cliOpts.delete = options.delete;
      }
      if (options.reset) {
        cliOpts.reset = options.reset;
      }
    ...
    if (scope === "global" || scope === "project") {
      ...
      if (scope === "global") {
        settingsOpts.global = true;
      }
      if (scope === "project") {
        settingsOpts.project = true;
      }
      if (options.list) {
        settingsOpts.list = options.list;
      }
      if (options.get) {
        settingsOpts.get = options.get;
      }
      if (options.set) {
        settingsOpts.set = options.set;
      }
      if (options.delete) {
        settingsOpts.delete = options.delete;
      }
      if (options.edit) {
        settingsOpts.edit = options.edit;
      }
      if (options.export) {
        settingsOpts.export = options.export;
      }
      if (options.import) {
        settingsOpts.import = options.import;
      }
      if (options.template) {
        settingsOpts.template = options.template;
      }
      if (options.explain) {
        settingsOpts.explain = options.explain;
      }
      if (options.reset) {
        settingsOpts.reset = options.reset;
      }
      if (options.verbose) {
        settingsOpts.verbose = options.verbose;
      }
      if (options.json) {
        settingsOpts.json = options.json;
      }
      ...
      const { selectedScope } = await inquirer.prompt([
        {
          ...
          choices: [
            {
              name: "CLI Configuration (API URL, timeout, retries)",
              value: "cli",
            },
    ...
    if (options.export || options.import || options.reset || options.delete) {
      logger.error(
        "The --export, --import, --reset, and --delete operations require --scope (cli, global, or project)"
      );
      process.exit(1);
    }
...
async function listConfig(config: ConfigManager): Promise<void> {
  ...
  logger.info(
    `${chalk.cyan("Default Environment:")} ${currentConfig.defaultEnv}`
  );
  ...
  if (currentConfig.token) {
    logger.info(
      `${chalk.cyan("Authentication:")} ${chalk.green("✓ Logged in")}`
    );
  } else {
    logger.info(
      `${chalk.cyan("Authentication:")} ${chalk.red("✗ Not logged in")}`
    );
  }
}
...
async function setConfig(
  ...
  // Validate key
  const validKeys = ["apiUrl", "defaultEnv", "timeout", "retries"];
  ...
  if (key === "timeout" && (parsedValue < 1000 || parsedValue > 300_000)) {
  ...
  logger.info(
    `${chalk.green("✓")} Set ${chalk.cyan(key)} = ${chalk.yellow(value)}`
  );
}
      ...
      const urlAnswer = await inquirer.prompt([
        {
          ...
          validate: (input: string) =>
            isValidUrl(input) || "Please enter a valid URL",
        },
      ]);
