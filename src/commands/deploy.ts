  ...
  try {
    ...
    if (!envConfig) {
      ...
      logger.error(
        ...
        Object.keys(projectConfig.environments ?? {}).join(", ")
      );
      process.exit(1);
    }
    ...
    const finalDeployment = await monitorDeployment(
      api,
      deployment.id,
      spinner
    );
...
async function handleRollback(
  ...
  try {
    ...
    if (!options.force) {
      ...
      logger.info(
        `  Created: ${chalk.cyan(new Date(previousDeployment.createdAt).toLocaleString())}`
      );
