...
async function listSettings(scope: 'global' | 'project', options: SettingsOptions): Promise<void> {
  ...
  if (scope === 'project' || (!options.global && !options.project)) {
    ...
    if (!projectSettings) {
      if (scope === 'project') {
        logger.warn('No project settings found. Not in a Cirron project directory?');
        return;
      }
    } else {
      if (options.json) {
...
function displaySettingsSection(title: string, section: any, maskSensitive = false): void {
  logger.info(chalk.cyan(`${title}:`));
  
  Object.entries(section).forEach(([key, value]) => {
    let displayValue = value;
    
    if (maskSensitive && key.toLowerCase().includes('token') && typeof value === 'string') {
      displayValue = value.substring(0, 8) + '*'.repeat(Math.max(0, value.length - 8));
    }
...
async function getSetting(key: string, scope: 'global' | 'project', options: SettingsOptions): Promise<void> {
  try {
    let value: any;
...
async function setSetting(keyValue: string, scope: 'global' | 'project', _options: SettingsOptions): Promise<void> {
  ...
  try {
    ...
    logger.info(`${chalk.green('✓')} Set ${chalk.cyan(key)} = ${chalk.yellow(value)} (${scope})`);
  } catch (error) {
    logger.error(`Failed to set setting '${key}':`, error);
  }
}

async function deleteSetting(key: string, scope: 'global' | 'project', _options: SettingsOptions): Promise<void> {
  try {
    if (scope === 'global') {
      const settings = settingsManager.loadGlobalSettings();
...
async function editSettings(scope: 'global' | 'project', _options: SettingsOptions): Promise<void> {
...
async function editGeneralSettings(settings: any, scope: 'global' | 'project'): Promise<void> {
  if (scope === 'global') {
    const answers = await inquirer.prompt([
      {
        type: 'select',
        ...
        choices: ['pytorch', 'pytorch-train', 'tensorflow', 'tensorflow-train', 'sklearn', 'sklearn-pipeline', 'custom'],
        loop: false
      },
      {
        type: 'confirm',
...
async function editAPISettings(settings: any): Promise<void> {
  ...
  answers.timeout = parseInt(answers.timeout, 10);
  answers.retries = parseInt(answers.retries, 10);
  ...
  const answers = await inquirer.prompt([
    {
      ...
      choices: ['cpu', 'cuda', 'gpu', 'transformer', 'xgboost', 'resnet', 'lstm', 'autoencoder'],
      loop: false
    },
    {
      type: 'confirm',
...
async function editTestSettings(settings: any): Promise<void> {
  ...
  answers.coverageThreshold = parseFloat(answers.coverageThreshold);
...
async function editDeploymentSettings(settings: any): Promise<void> {
  ...
  answers.healthCheckTimeout = parseInt(answers.healthCheckTimeout, 10);
  Object.assign(settings.deployment, answers);
}

async function exportSettings(filePath: string, _scope: 'global' | 'project', options: SettingsOptions): Promise<void> {
  try {
    const type = options.global ? 'global' : options.project ? 'project' : 'combined';
    settingsManager.exportSettings(type, filePath);
    logger.info(`${chalk.green('✓')} Settings exported to ${chalk.cyan(filePath)}`);
  } catch (error) {
    logger.error(`Failed to export settings: ${error}`);
  }
}

async function importSettings(filePath: string, _scope: 'global' | 'project', _options: SettingsOptions): Promise<void> {
  try {
    settingsManager.importSettings(filePath);
    logger.info(`${chalk.green('✓')} Settings imported from ${chalk.cyan(filePath)}`);
  } catch (error) {
    logger.error(`Failed to import settings: ${error}`);
  }
}

async function applyTemplate(templateName: string, _options: SettingsOptions): Promise<void> {
  try {
    ...
    if (confirm.apply) {
      ...
      logger.info(`${chalk.green('✓')} Applied template: ${chalk.cyan(template.name)}`);
    } else {
...
async function explainSetting(key: string, options: SettingsOptions): Promise<void> {
  try {
    ...
    logger.info(`${chalk.yellow('Source')}: ${resolution.source.type}${resolution.source.file ? ` (${path.basename(resolution.source.file)})` : ''}`);
    ...
    if (resolution.overriddenBy && resolution.overriddenBy.length > 0) {
      ...
      resolution.overriddenBy.forEach((source, index) => {
        logger.info(`  ${index + 1}. ${source.type}${source.file ? ` (${path.basename(source.file)})` : ''}: ${source.value}`);
      });
      logger.info(`  ${resolution.overriddenBy.length + 1}. ${chalk.green(resolution.source.type)}${resolution.source.file ? ` (${path.basename(resolution.source.file)})` : ''}: ${chalk.green(resolution.value)} ${chalk.dim('← final value')}`);
    }
...
async function resetSettings(scope: 'global' | 'project', _options: SettingsOptions): Promise<void> {
  const confirm = await inquirer.prompt([
    {
      type: 'confirm',
      name: 'reset',
...
async function interactiveSettings(scope: 'global' | 'project', options: SettingsOptions): Promise<void> {
  const choices = [
